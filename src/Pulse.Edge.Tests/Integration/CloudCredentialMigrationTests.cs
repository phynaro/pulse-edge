...
public sealed class CloudCredentialMigrationTests(PulseEdgeAppFactory factory) : IClassFixture<PulseEdgeAppFactory>
{
    ...
    [Fact]
    public async Task Startup_migration_encrypts_plaintext_claim_secret_and_pairing_token_when_api_key_is_empty()
    {
        await factory.ResetDatabaseAsync();
        _ = factory.CreateClient(); // ensure the protector is configured
        ...
        // Simulate a pre-2C device mid-pairing: ClaimSecret/PairingToken were generated by
        // CloudProvisioningService before the cloud ever issued an ApiKey, so ApiKey is empty
        // but the other two are real plaintext secrets, bypassing the converter.
        await using (var db = new QueueDbContext(dbPath))
        {
            await db.Database.EnsureCreatedAsync();
            await db.Database.ExecuteSqlRawAsync(
                "INSERT INTO DeviceConfigs (Id, ClaimSecret, PairingToken, ApiKey, CloudEndpoint, Version, IsSyncEnabled, CloudStatus, PairingShortCode, PairingBaseUrl, SerialNumber, OrganizationId, OrganizationName, SiteId, SiteName, CloudEdgeId) " +
                "VALUES ('dev-1', 'pending-claim-secret', 'pending-pairing-token', '', 'https://cloud', '1.0.0', 1, 'PendingApproval', '', '', 'SN', '', '', '', '', '')");
        }

        // The migration helper — same code the app runs at startup.
        await CloudCredentialMigration.MigrateAsync(dbPath);

        await using (var raw = new QueueDbContext(dbPath))
        {
            var row = (await raw.Database
                .SqlQueryRaw<RawCreds>(
                    "SELECT ApiKey, ClaimSecret, PairingToken FROM DeviceConfigs LIMIT 1")
                .ToListAsync()).Single();

            Assert.Equal(string.Empty, row.ApiKey);
            Assert.NotEqual("pending-claim-secret", row.ClaimSecret);
            Assert.NotEqual("pending-pairing-token", row.PairingToken);
            Assert.True(SecretProtection.Protector.IsProtected(row.ClaimSecret));
            Assert.True(SecretProtection.Protector.IsProtected(row.PairingToken));
        }
        await using (var ef = new QueueDbContext(dbPath))
        {
            var config = await ef.DeviceConfigs.AsNoTracking().FirstAsync();
            Assert.Equal(string.Empty, config.ApiKey);
            Assert.Equal("pending-claim-secret", config.ClaimSecret);
            Assert.Equal("pending-pairing-token", config.PairingToken);
        }
    }

    private sealed class RawCreds
    {
