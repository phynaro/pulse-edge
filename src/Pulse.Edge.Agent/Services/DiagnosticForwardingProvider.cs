...
public sealed class DiagnosticForwardingProvider : ILoggerProvider, ISupportExternalScope, IHostedService
{
    ...
    public DiagnosticForwardingProvider() => _client.DefaultRequestHeaders.Add("X-Pulse-Diagnostic-Key", DiagnosticBridgeKey.LoadOrCreate());

    public ILogger CreateLogger(string categoryName) => new ForwardingLogger(categoryName, _queue.Writer, () => _scopeProvider);
    ...
    private sealed class ForwardingLogger(string category, ChannelWriter<ForwardedDiagnostic> writer, Func<IExternalScopeProvider> scopes) : ILogger
    {
        ...
        private static string LevelName(LogLevel level) => level switch { LogLevel.Critical => "Critical", LogLevel.Error => "Error", LogLevel.Warning => "Warning", _ => "Information" };
    }
