        ...
        CancellationToken ct)
    {
        ...
        foreach (var dp in dueDps)
        {
            try
            {
                ...
                _logger.LogInformation("[Siemens S7 Read] Address: {Address} | Raw: {Raw} | Processed: {Value}", dp.Address, rawVal, processedVal);
                ...
                if (!string.IsNullOrEmpty(dp.DataSourceId) && !string.IsNullOrEmpty(dp.Metric))
                {
                    if (await _storageService.IsDataSourceEnabledAsync(dp.DataSourceId))
                    {
                        ...
                        _logger.LogInformation("[Queue Buffer] Enqueued Siemens S7 telemetry | Stream: {Source} Metric: {Metric}", dp.DataSourceId, dp.Metric);
                    }
                }
            }
            catch (Exception ex)
