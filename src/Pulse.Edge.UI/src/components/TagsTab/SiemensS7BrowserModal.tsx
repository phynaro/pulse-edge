...
interface SiemensS7BrowserModalProps {
  isOpen: boolean;
  onClose: () => void;
  adapterId: string;
...
export default function SiemensS7BrowserModal({
  isOpen,
  onClose,
  adapterId,
  adapters,
  ...
  const [configuringTags, setConfiguringTags] = useState<S7ConfiguringTag[]>([]);
  ...
  const fetchPlcTags = async (targetAdapterId: string) => {
    ...
    setError('');
    try {
      const res = await fetch('/api/adapters/siemens-s7/browse', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        setError(data.message || 'Failed to discover S7 PLC tags.');
      }
    } catch (err) {
  ...
  const handleNextStep = () => {
    ...
    setConfiguringTags(selectedList.map(tag => {
      ...
      return {
        ...
        scanIntervalMs: 1000,
        description: `Discovered Siemens S7 tag: ${tag.name}`
      };
    }));
    setStep(2);
  };

  const handleUpdateConfiguringTag = (index: number, key: keyof S7ConfiguringTag, value: string | number) => {
    setConfiguringTags(prev => {
      const next = [...prev];
      next[index] = { ...next[index], [key]: value } as S7ConfiguringTag;
      return next;
    });
  };
  ...
  const handleSaveTags = async () => {
    ...
    try {
      ...
      if (successes > 0) toast.success(`Successfully registered ${successes} Siemens S7 tags.`);
  ...
  return (
    <ModalShell
      title="Siemens S7 PLC Tag Discoverer"
    ...
    >
      {step === 1 ? (
          ...
          <div className="browser-left-pane">
            ...
            {loading && discoveredTags.length === 0 ? (
              <div className="browser-loading">
                ...
                <span className="browser-loading-text">Connecting to PLC and querying tag database...</span>
              </div>
            ) : error ? (
              ...
              <div className="browser-node-section">
                ...
                {filteredDiscoveredTags.length === 0 ? (
                  <div className="browser-folder-empty">
                    {discoveredTags.length === 0 ? 'No Siemens S7 tags were discovered. Ensure PUT/GET communication is allowed.' : 'No symbols match your filter.'}
                  </div>
                ) : (
          ...
          <div className="browser-right-pane">
            ...
            {selectedCount === 0 ? (
              <div className="browser-empty-right">
                ...
                <span className="browser-empty-text">Select Siemens S7 symbols on the left to add them as registry tags.</span>
              </div>
            ) : (
        ...
        <div className="browser-config-body">
          <span className="browser-config-step-label">Step 2: Configure Siemens S7 Tag Parameters</span>
          <div className="browser-config-wrap">
            <table className="browser-config-table">
              <thead>
                <tr>
                  <th>Siemens S7 Address</th>
