...
- **`QueueStorageService`** (`Pulse.Edge.Storage`) — the single SQLite store (EF Core, `QueueDbContext`). Holds queued telemetry, OEE channels + outbox, adapters, data sources, data points, stream templates, local users, audit + diagnostic events. `InitializeAsync()` runs at startup before hosted services to avoid races. The telemetry table (`QueueTelemetry`) is dropped only when a legacy pre-merged-metrics schema is detected (missing the `MetricsJson` column); on an already-migrated device it is never dropped, so buffered telemetry survives restarts — this is the durability boundary — Roadmap Phase 4 hardens it further (WAL, retention, disk thresholds).
