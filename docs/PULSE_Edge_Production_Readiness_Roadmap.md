...
| 2 | Security candidate | In progress | Engineering | TBD | G2 | All implementation slices merged: 2A (PR #12), 2B (PR #13), 2C (PR #15), 2D (PR #17: authz matrix + completeness guard, backup abuse tests, loopback-scoped TLS trust + bypass guard, CodeQL, exception register R-008..R-011). Remaining before the gate: run the [G2 smoke runbook](PULSE_Edge_G2_Smoke_Runbook.md) (five 2B+2C manual smokes) on the staging box, promote `CodeQL`/`Secret scan` to required checks, then hold the G2 gate review. |
...
- [x] Encrypt cloud keys at rest. *(Narrowed by R-008: protocol/device credentials are deliberately left plaintext and backup-recoverable; only cloud keys — `ApiKey`, `ClaimSecret`, `PairingToken` — are encrypted at rest.)* Evidence: Slice 2C — EF value converter + DataProtection key ring shared by Api/Agent, startup migration for legacy plaintext; `CloudCredentialEncryptionTests`, `CloudCredentialMigrationTests`, `DataProtectionSecretProtectorTests`.
- [x] Redact secrets from API responses, logs, exports, and support bundles. *(API responses: Slice 2A B-01. Diagnostic log sink + backup export: Slice 2C — `DiagnosticRedactionTests`, `BackupHardeningTests` export-exclusion guard. Gate-level sweep evidence remains Slice 2D.)*
...
- [x] Add credential and certificate rotation procedures. Evidence: [PULSE_Edge_Credential_Rotation.md](PULSE_Edge_Credential_Rotation.md) (Slice 2C; accuracy verified against code in review).
...
- Slice 2C (secrets at rest, redaction & rotation): merged ([PR #15](https://github.com/phynaro/pulse-edge/pull/15)) — [design spec](superpowers/specs/2026-07-14-slice-2c-secrets-at-rest-design.md), [implementation plan](superpowers/plans/2026-07-14-slice-2c-secrets-at-rest.md). B-07: cloud creds encrypted at rest via an EF value converter + DataProtection key ring (`<data dir>/dp-keys`, shared by the Api and Agent processes) with an idempotent startup migration for legacy plaintext (gate: any of the three fields unprotected, covering mid-pairing devices). B-09: backup export cloud-cred exclusion locked in by test; restore endpoints get a pre-parse 5 MB cap via middleware (fixed-length + chunked). B-10: diagnostic-sink redaction of labelled credential values test-asserted (no regex change needed). B-12: [credential rotation runbook](PULSE_Edge_Credential_Rotation.md) + LUKS/TPM data-at-rest note; B-11 accepted as R-010. Five per-task reviews (two fix waves: Agent protector sharing; migration gate widening) + whole-branch review (Opus): ready to merge, no Critical/Important findings. Backend suite 101/101; `--locked-mode` restore verified. Accepted side effect: one-time UI re-login on first boot after upgrade (shared DataProtection key ring).
- Slice 2D (gate verification & evidence): merged ([PR #17](https://github.com/phynaro/pulse-edge/pull/17), squash `4d2addf`) — [design spec](superpowers/specs/2026-07-14-slice-2d-gate-verification-design.md), [implementation plan](superpowers/plans/2026-07-14-slice-2d-gate-verification.md). Restore endpoints gain declarative `RequireAuthorization` (metadata-asserted). Backup abuse tests: malformed/tampered rejection (non-JSON, wrong format/version, stale checksum, dangling refs, inspect/apply parity + no-partial-write); found and fixed a 500 (NRE in `ConfigurationBackupService.Inspect` on null collections → clean 400). TLS: Agent diagnostic forwarding narrowed from accept-any-cert to loopback-only (`LoopbackCertificateTrust`, R-011); `TlsBypassGuardTests` source guard (4 bypass markers, bite-checked) forbids any new bypass in production code. CodeQL workflow added (csharp + javascript-typescript, build-mode none) — first runs green, 0 alerts. [Security exception register](PULSE_Edge_Security_Exception_Register.md) (R-008..R-011) + [G2 smoke runbook](PULSE_Edge_G2_Smoke_Runbook.md). Six per-task reviews (two fix waves: broadened TLS-bypass markers; runbook dp-keys path) + whole-branch review (Opus): ready to merge, no Critical/Important. Backend suite 113/113; `--locked-mode` + `--warnaserror` verified.
- Endpoint authorization test report: `AuthorizationMatrixTests` — full matrix, 65 endpoints classified (anonymous→401 / ReadOnly→403 per class) with a bidirectional completeness guard (unclassified endpoint or dead row fails the suite). Superseded the 5-endpoint version from PR #12; merged in PR #17.
- Security scan: dependency (NuGet audit + pnpm audit, since G1) + secret scan (gitleaks `secrets` job, since G1; GitGuardian app) + static analysis (CodeQL, PR #17 — [first run green, 0 open alerts](https://github.com/phynaro/pulse-edge/actions/runs/29343756795)). Remaining for the gate: promote `CodeQL` + `Secret scan` to required branch-protection checks.
- Backup abuse test report: `BackupHardeningTests` — oversized fixed-length + chunked (Slice 2C), malformed/tampered/dangling-ref/no-partial-write (Slice 2D, PR #17).
- Security exception register: [PULSE_Edge_Security_Exception_Register.md](PULSE_Edge_Security_Exception_Register.md) (R-008, R-009, R-010, R-011).
- Windows installer fix (out-of-slice, [PR #18](https://github.com/phynaro/pulse-edge/pull/18)): Inno/WiX installers predated Slice 2B and wrote `http://*` into `config.json`, making every authenticated call 401 on a fresh Windows install (Secure cookie dropped over HTTP). Both installers now configure `https://*`, https shortcuts, and a cert-tolerant health probe. Windows deployment remains deferred (R-006); L-004 updated.
...
| 2026-07-14 | G2 | In progress | Slice 2C implemented on `slice-2c-secrets-at-rest`: cloud creds encrypted at rest (EF converter + DataProtection, key ring shared Api/Agent), idempotent legacy-plaintext migration (any-of-three gate), pre-parse 5 MB restore cap incl. chunked, redaction + backup-export exclusion test-asserted, rotation runbook + R-010. Five per-task reviews with two owner-decided fix waves (Agent protector sharing after the MultiPort gap finding; migration gate widened for mid-pairing devices); whole-branch review (Opus): ready to merge, no Critical/Important. 101/101 backend tests; clean `--warnaserror` build; `--locked-mode` restore verified. Accepted: one-time UI re-login on first post-upgrade boot. | Merge the Slice 2C PR after CI, then execute Slice 2D (gate test sweep: security scans, backup abuse tests incl. malformed/tampered schema validation, exception register, G2 review). Run the 2B+2C manual smokes before the gate. |
| 2026-07-14 | G2 | In progress | Slice 2C merged to `main` ([PR #15](https://github.com/phynaro/pulse-edge/pull/15), squash `de31120`) with all Quality checks green. G2 implementation slices 2A–2C are complete; only the gate-evidence work remains. | Plan and execute Slice 2D: security scan evidence, backup abuse tests (malformed/oversized/tampered — oversized already covered by `BackupHardeningTests`), security exception register, restore-endpoint `RequireAuthorization` follow-up, 2B+2C manual smokes, then the G2 gate review. |
| 2026-07-16 | G2 | In progress | Slice 2D merged to `main` ([PR #17](https://github.com/phynaro/pulse-edge/pull/17), squash `4d2addf`): full authz matrix (65 endpoints + completeness guard), malformed/tampered backup rejection (incl. a real 500 fixed in `Inspect`), loopback-scoped TLS trust + source bypass guard (R-011), CodeQL (first runs green, 0 alerts), exception register + smoke runbook. 113/113 tests; whole-branch review (Opus) clean. Out-of-slice: Windows installer HTTPS fix merged ([PR #18](https://github.com/phynaro/pulse-edge/pull/18)) after a fresh Windows install surfaced all-401 (installers predated 2B, wrote `http://*` config; Secure cookie dropped over HTTP). Follow-up filed: occurrence-level TLS-guard allowlist. | Owner runs the [G2 smoke runbook](PULSE_Edge_G2_Smoke_Runbook.md) on the staging box → results to `docs/readiness-evidence/`; promote `CodeQL` + `Secret scan` to required branch-protection checks; then hold the G2 gate review (check gate boxes with evidence links). |
...
| R-010 | 2026-07-14 | Decision | The Windows-only `config.json.sha256` integrity check (B-11) stays an **unkeyed** hash — it detects accidental corruption, not authenticated tampering. Accepted while Windows deployment is deferred (R-006); reinstate a keyed MAC (e.g. HMAC keyed by the DataProtection key ring) when Windows returns to the supported scope. Recorded in [PULSE_Edge_Pilot_Known_Limitations.md](PULSE_Edge_Pilot_Known_Limitations.md) alongside L-007. | Project owner | Accepted | Accepted |
...
| D-001 | 2026-07-19 | Decision | OEE ingestion implemented per know-how/cloud_oee_ingestion.md: dedicated OeeSyncService (independent of telemetry SyncService), durable OeeOutboxMessages with per-channel seq, dead legacy QueueEvents subsystem removed. OEE channels are NOT yet included in the configuration backup payload (requires a backup format-version bump) — follow-up item. (`feature/edge-oee-ingestion`, PR at open time) | Engineering | Accepted | Accepted |
| D-002 | 2026-07-19 | Decision | Fixed a latent data-loss bug in `QueueStorageService.InitializeAsync()`: `QueueTelemetry` (the store-and-forward telemetry buffer) was unconditionally `DROP TABLE`'d and recreated on every process startup since the merged-metrics schema migration, destroying buffered telemetry on every real device reboot. Now schema-detection-guarded — the drop only runs when the pre-merged-metrics legacy schema is detected (missing `MetricsJson`); an already-migrated database is never dropped. A follow-up idempotent `ALTER TABLE ADD COLUMN QualitiesJson` patch also covers a partial-schema case (`MetricsJson` present, `QualitiesJson` missing). Found during OEE test-determinism hardening on `feature/edge-oee-ingestion`. | Engineering | Accepted | Accepted |
