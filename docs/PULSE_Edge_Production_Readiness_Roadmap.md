...
| 2 | Security candidate | In progress | Engineering | TBD | G2 | Slices 2A (PR #12), 2B (PR #13) merged; Slice 2C (secrets at rest: B-07/B-09/B-10/B-12, B-11→R-010) implemented + reviewed, PR open. Slice 2D (gate test sweep + review) remains before the gate, plus manual smokes: 2B (published-build HTTPS, `systemd-analyze security`) and 2C (pre-2C upgrade reconnects to cloud; `dp-keys` 0700; MultiPort two-process pairing). |
